...
class ProductsTab:
    def __init__(self, parent, main_app):
        ...
        self.data = []
        self.columns = ["產品編號", "產品名稱", "分類", "單價", "庫存", "供應商", "備註"]
        # ---- 先定義屬性（避免尚未定義就被使用） ----
        self.required_fields = ["產品編號","產品名稱","分類","單價"]
        self.dropdown_options = {"分類": ["電子產品","家具","文具","服裝","食品","其他"]}
        self.multiline_fields = ["備註"]
    ...
    def _setup_scrollable_edit_area(self, parent):
        ...
        self._bind_mousewheel_to(canvas)
        return inner

    def _make_input_widget(self, parent, col):
        if col in self.dropdown_options:
            cb = ttk.Combobox(parent, values=self.dropdown_options[col], state="readonly")
            return cb, "combo"
        if col in self.multiline_fields:
            txt = tk.Text(parent, height=3, wrap="word"); return txt, "text"
        ent = ttk.Entry(parent); return ent, "entry"

    def _set_widget_value(self, w, t, val):
        if t == "text":
            w.delete("1.0", "end"); w.insert("1.0", "" if val is None else str(val)); return
        if t == "combo": w.set("" if val is None else str(val)); return
        w.delete(0, tk.END); w.insert(0, "" if val is None else str(val))

    def _get_widget_value(self, w, t):
        if t == "text": return w.get("1.0","end-1c")
        return w.get()
    ...
    def _load_row_to_form(self, r):
        ...
        for i, col in enumerate(self.columns):
            w, t = self.inputs[col]
            self._set_widget_value(w, t, vals[i] if i < len(vals) else "")
        self.current_row = r
    ...
    def setup_ui(self):
        ...
        ttk.Button(btns, text="新增" + ("" if "ProductsTab"!="OrdersTab" else "訂單"), command=self.add_row).pack(side=tk.LEFT, padx=4)
        ttk.Button(btns, text="修改" + ("" if "ProductsTab"!="OrdersTab" else "訂單"), command=self.update_row).pack(side=tk.LEFT, padx=4)
    ...
    def refresh_sheet(self):
        ...
        self._update_row_heights_for_wrap()
        self.apply_sheet_styles()
        self.sheet.redraw()

    def add_row(self):
        vals = [self._get_widget_value(*self.inputs[col]) for col in self.columns]
        if not self._validate(vals): return
        self.data.append(vals)
        self.refresh_sheet(); self.clear_form()

    def update_row(self):
        r = self.get_selected_row()
        if r is None or not (0 <= r < len(self.data)):
            messagebox.showwarning("警告", "請先選取要修改的列"); return
        vals = [self._get_widget_value(*self.inputs[col]) for col in self.columns]
        if not self._validate(vals): return
    ...
    def delete_row(self):
        ...
        if 0 <= r < len(self.data): del self.data[r]; self.refresh_sheet()

    def clear_all_data(self):
        if messagebox.askyesno("確認","確定清除全部資料？"):
            self.data.clear(); self.refresh_sheet()

    def clear_form(self):
        for col in self.columns:
            w, t = self.inputs[col]
            if t == "text": w.delete("1.0","end")
            elif t == "combo": w.set("")
            else: w.delete(0, tk.END)

    def set_data(self, rows):
        self.data = rows or []; self.refresh_sheet()
    ...
    def export_to_excel(self):
        ...
        wb=Workbook(); ws=wb.active; ws.title="產品管理"
        ...
        for i,w in enumerate([12,18,12,10,10,14,24], start=1):
